...
def run_task(data_fetcher: DataFetcher, sensorentity_updator: SensorentityUpdator):
    try:
        ...
        for i in range(0, len(user_id_list)):
            ...
            sensorentity_updator.update("sensor.electricity_95598" + profix,  balance_list[i], {"unit_of_measurement": "CNY", "is_pay": balance_list_pay[i], "need_pay": balance_list_need_pay[i], "last_electricity_usage": last_daily_usage_list[i], "yearly_electricity_usage": yearly_usage_list[i], "yearly_electricity_charge": yearly_charge_list[i], "thismonth_electricity_usage": thismonth_usage_list[i],"last_date_list":last_date_list[i], "update_time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")},)
